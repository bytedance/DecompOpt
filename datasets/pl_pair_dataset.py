...
def get_decomp_dataset(config, **kwargs):
    ...
    root = config.path
    if name == 'pl':
        dataset = DecompPLPairDataset(root, mode=config.mode, load_retrieved_feat=getattr(config.transform, 'add_retrieved_feat', False),
                                        retrieved_filepath=getattr(config, 'retrieved_filepath', None),
...
class DecompPLPairDataset(Dataset):
    ...
    def __init__(self, raw_path, transform=None, mode='full', retrieved_filepath=None,
                 include_dummy_atoms=False, kekulize=True, load_retrieved_feat=False, version='v1'):
        super().__init__()
        ...
        if self.load_retrieved_feat:
            print(f"loading: {retrieved_filepath}")
            self.retrieved_frags = torch.load(retrieved_filepath)
    ...
    def __getitem__(self, idx):
        ...
        data.id = idx
        if self.load_retrieved_feat:
            data.retrieved_frags = self.retrieved_frags[idx]
