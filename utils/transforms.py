...
class FeaturizeArmAtom(object):

    def __init__(self, ligand_featurizer, protein_featurizer=None, padding_protein_feature_dim=None):
        super().__init__()
        self.ligand_featurizer = ligand_featurizer
        self.protein_featurizer = protein_featurizer
        self.padding_protein_feature_dim = padding_protein_feature_dim

    def __call__(self, data: ProteinLigandData):
        for arm in data.selected_sim_arms:           
            self.ligand_featurizer(arm)
            if self.protein_featurizer is not None:
                self.protein_featurizer(arm)
            if self.padding_protein_feature_dim is not None:
                arm.protein_atom_feature = torch.cat([arm.protein_atom_feature, \
                    torch.zeros((arm.protein_atom_feature.shape[0], self.padding_protein_feature_dim), dtype=torch.float32)], -1)
        return data
...
class AddDecompIndicator(object):
    def __init__(self, max_num_arms=10, global_prior_index=None, add_ord_feat=False,
                 add_to_protein=True, add_to_ligand=True, add_retrieved_feat=False):
        super().__init__()
        self.max_num_arms = max_num_arms
        self.global_prior_index = global_prior_index
        self.add_ord_feat = add_ord_feat
        self.add_retrieved_feat = add_retrieved_feat
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        if self.add_to_ligand:
            ...
            if self.add_retrieved_feat:
                data.ligand_arm_group_idx = data.ligand_atom_mask.clone()
                data.ligand_arm_atom_mask = (data.ligand_atom_mask >= 0).long()
                if len(data.selected_sim_arms) != data.num_arms:
                    print(f"data.id={data.id} misalgnment on num_arms.")
                else:
                    for arm_idx in range(data.num_arms):
                        sim_arm = data.selected_sim_arms[arm_idx]
                        sim_arm_ind = F.one_hot((torch.ones(size=(sim_arm.ligand_atom_feature.shape[0],))).long(), num_classes=2)
                        sim_arm_scaffold_index = F.one_hot(arm_idx * torch.ones(size=(sim_arm.ligand_atom_feature.shape[0],)).long(), self.num_classes)

                        if self.add_ord_feat:
                            # Note: aux feature should be independent to pos
                            sim_arm.ligand_atom_aux_feature = torch.cat([sim_arm_scaffold_index, sim_arm_ind], -1)
                        else:
                            sim_arm.ligand_atom_aux_feature = sim_arm_ind
...
class FeaturizeLigandBond(object):
    ...
    def __call__(self, data: ProteinLigandData):
        if self.mode == 'fc':
            if hasattr(data, 'ligand_atom_mask'):
            ...
            for i in range(data.num_arms):
                n_atoms = (data.ligand_atom_mask == i).sum()
                arm_atom_idx = (data.ligand_decomp_mask == i).nonzero()[:, 0]
                full_dst = torch.repeat_interleave(sca_atom_idx, n_atoms)
                full_src = arm_atom_idx.repeat(n_atoms_sca)
                mask = full_dst != full_src
                full_dst, full_src = full_dst[mask], full_src[mask]
                all_full_src.append(full_src)
                all_full_dst.append(full_dst)
        ...
        # if self.add_retrieved_feat and hasattr(data, 'selected_sim_arms'):
        #     for arm in data.selected_sim_arms:
        #         self(arm)
        return data


class FeaturizeArmBond(object):
    def __init__(self, bond_featurizer):
        super().__init__()
        self.bond_featurizer = bond_featurizer

    def __call__(self, data: ProteinLigandData):
        assert hasattr(data, 'selected_sim_arms')
        for arm in data.selected_sim_arms:
            self.bond_featurizer(arm)
        return data
