...
def reconstruct_from_generated_with_bond_scaffoldfc(xyz, atomic_nums, bond_index, bond_type,
                                                    decomp_mask, check_validity=True):
    n_atoms = len(atomic_nums)
    ...
    num_bonds = len(bond_type)
    random_order_bond_indices = np.arange(num_bonds)
    np.random.shuffle(random_order_bond_indices)
    random_order_bond_indices = random_order_bond_indices.tolist()

    bond_type = bond_type[random_order_bond_indices]
    bond_index = [np.array(bond_index[0])[random_order_bond_indices].tolist(), 
                  np.array(bond_index[1])[random_order_bond_indices].tolist()]
    forbidden_scaffold_arm_pairs = set()
    ...
    for i, type_this in enumerate(bond_type):
        ...
        if node_i < node_j:
            ...
            node_i_part = decomp_mask[node_i]
            node_j_part = decomp_mask[node_j]
            if node_i_part == -1 and node_j_part != -1: 
                # i-scaffold, j-arm 
                decomp_pair_mark = (node_i_part, node_j_part)
                # print(decomp_pair_mark)
                if decomp_pair_mark in forbidden_scaffold_arm_pairs:
                    print("#### Forbidden!")
                    continue 
                else:
                    forbidden_scaffold_arm_pairs.add(decomp_pair_mark)
                    if type_this != 1:
                        print(f"#### force the {type_this} to be SINGLE")
                    type_this = 1

            if node_i_part != -1 and node_j_part == -1:
                # i-arm, j-scaffold
                decomp_pair_mark = (node_j_part, node_i_part)
                # print(decomp_pair_mark)
                if decomp_pair_mark in forbidden_scaffold_arm_pairs:
                    print("#### Forbidden!")
                    continue
