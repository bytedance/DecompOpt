...
def split_arms_scaffold(mol, atom_map_id, target_cluster_num):
    ...
    '''
    assert target_cluster_num >= 1 and len(atom_map_id) >= target_cluster_num
    ...
    # to do: assign farthest fragment as scaffold center 
    if len(non_terminal_ids) != 0:
        ...
        scaffold_num = 1
        # remove scaffold center from centers to be assigned
        del fragment_centers[assign_scaffold_frag]
        del non_terminal_ids[assign_scaffold_frag]
    else:
        scaffold_center = []
        scaffold_id = []
        scaffold_num = 0
    ...
    if target_cluster_num == len(atom_map_id):
        ...
        return terminal_ids, sum([non_terminal_ids, scaffold_id], []), terminal_clustering_num, scaffold_num
        ...
        if len(fragment_centers) == 0:
            # merge ternimal arms and scaffold fragments
            fragment_centers = terminal_centers
            non_terminal_ids = terminal_ids
            fragment_mapping = distance_matrix(fragment_centers, terminal_centers)
            fragment_mapping[np.where(fragment_mapping == 0)] = INF
        else:
            fragment_mapping = distance_matrix(fragment_centers, terminal_centers)

        # do not merge disconnected fragments
        to_be_merge = True
        loop = 0
        while to_be_merge:
            flag = False
            loop += 1
            if loop > len(fragment_centers) * len(terminal_centers):
                # should merge ternimal arms and scaffold fragments
                fragment_mapping = distance_matrix(fragment_centers, terminal_centers)

            mid = np.array(np.where(fragment_mapping == fragment_mapping.min()))[:,0]
            if mid[1] != len(terminal_centers) - 1 and loop <= len(fragment_centers) * len(terminal_centers):
                # try to merge with terminal arms
                if is_terminal_frag(mol, np.concatenate([non_terminal_ids[mid[0]], terminal_ids[mid[1]]])):
                    flag = True
            else:
                # try to merge with scaffold fragments
                if '.' not in Chem.MolToSmiles(get_frag_by_atom(mol, np.concatenate([atom_map_id[mid[0]], terminal_ids[mid[1]]]))):
                    flag = True
            
            if flag:
                atom_map_id.append(np.concatenate([non_terminal_ids[mid[0]], terminal_ids[mid[1]]]).tolist())
                atom_map_id.remove(non_terminal_ids[mid[0]])
                atom_map_id.remove(terminal_ids[mid[1]])
                to_be_merge = False
            else:
                fragment_mapping[mid[0], mid[1]] = INF
...
def extract_submols(mol, pocket_list, debug=False, verbose=False):
    ...
    arm_atom_ids, scaffold_atom_ids, arm_num, scaffold_num = split_arms_scaffold(mol, frags_atom_idx_list, min(terminal_mask.sum(), len(pocket_list)) + int(len(non_arm_frag_idx) > 0))
    all_submols = []
    sub_mol_centers = []
    valid_pocket_id = []
    for atom_id in arm_atom_ids:
        # assert len(atom_id) > 0
        sub_mol = get_frag_by_atom(mol, atom_id)
        all_submols.append(sub_mol)
        sub_mol_centers.append(np.mean(sub_mol.GetConformer(0).GetPositions(), 0).tolist())
    if scaffold_num > 0:
        sub_mol = get_frag_by_atom(mol, scaffold_atom_ids[0])
        all_submols.append(sub_mol)

    pocket_centers = []
    for pocket_idx, pocket in enumerate(pocket_list):
        pocket_centers.append(np.mean(centers, 0).tolist())
    
    assignment = distance_matrix(sub_mol_centers, pocket_centers)
    assigned = 0
    valid_pocket_id = []
    while assigned < len(sub_mol_centers):
        min_idx = assignment.argmin()
        frag_id = int(min_idx / len(pocket_centers))
        pocket_id = min_idx % len(pocket_centers)
        valid_pocket_id.append(pocket_id)
        assignment[:, pocket_id - 1] = INF
        assignment[frag_id, :] = INF
        assigned += 1

    all_submol_atom_idx = arm_atom_ids + scaffold_atom_ids
    return all_frag_centroid, all_submol_atom_idx, all_submols, valid_pocket_id
