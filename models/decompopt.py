...
class DecompScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim, num_classes,
        ...
        self.add_retrieved_feat = getattr(config, 'add_retrieved_feat', False)
        ...
        if self.add_retrieved_feat:
            ligand_atom_feature_dim = self.hidden_dim + 2
        ...
        if self.add_retrieved_feat:
            self.fusion_net_type = config.fusion_net.model_type if hasattr(config, 'fusion_net') else config.model_type
            self.fusion_net = get_refine_net(self.fusion_net_type, getattr(config, 'fusion_net', config))
            self.fusion_atom_emb = nn.Linear(8, self.hidden_dim)
            self.fusion_bond_emb = nn.Linear(self.num_bond_classes, self.hidden_dim)
            self.fusion_mlp = MLP(2 * self.hidden_dim, self.hidden_dim, self.hidden_dim, 2, 
                                    norm=config.norm, act_fn=config.act_fn)
            self.fusion_protein_atom_emb = nn.Linear(protein_atom_feature_dim, self.hidden_dim)
            self.fusion_protein_mlp = MLP(2 * self.hidden_dim, self.hidden_dim, self.hidden_dim, 2, 
                                    norm=config.norm, act_fn=config.act_fn)

        if self.add_retrieved_feat:
            protein_atom_feature_dim = self.hidden_dim
    ...
    def forward(self, protein_pos, protein_v, batch_protein,
                ...
                arm_fc_bond_type, 
                batch_arm_bond,
                ligand_arm_group_idx, ligand_arm_atom_mask,
                arm_outputs,
                arm_protein_pos, arm_protein_v, arm_protein_batch, arm_pocket_atom_masks, arm_to_ligand_index,

                prior_centers, prior_stds, batch_prior,
                ligand_fc_bond_index, init_ligand_fc_bond_type,
                mask_arm_atom=None, arms_p_index_in_ctx=None, # for sampling
                protein_group_idx=None, ligand_group_idx=None, prior_group_idx=None,
                ligand_atom_mask=None, time_step=None, return_all=False):

        init_ligand_v = F.one_hot(init_ligand_v, self.num_classes).float()

        if self.add_retrieved_feat:
            init_ligand_v_feat = self.fusion_atom_emb(init_ligand_v)
            h_protein = self.fusion_protein_atom_emb(protein_v)

            if arm_outputs is None:
                h_arm_protein = self.fusion_protein_atom_emb(arm_protein_v)
                arm_v = F.one_hot(arm_v, self.num_classes).float()
                arm_v_feat = self.fusion_atom_emb(arm_v)

            if self.fusion_net_type == 'uni_o2_bond':
                # compose ligand and protein feature
                h_all, pos_all, batch_all, mask_ligand, mask_ligand_atom, p_index_in_ctx, l_index_in_ctx = compose_context(
                    h_protein=h_protein,
                    h_ligand=init_ligand_v_feat,
                ...
                ligand_bond_type = F.one_hot(init_ligand_fc_bond_type, num_classes=self.num_bond_classes).float()
                ligand_h_bond = self.ligand_bond_emb(ligand_bond_type)
                ligand_outputs = self.fusion_net(
                    h=h_all, x=pos_all, group_idx=None,
                    bond_index=bond_index_in_all, h_bond=ligand_h_bond,
                ...
                if arm_outputs is None:
                    # compose arms and protein featrue
                    h_all, pos_all, batch_all, mask_arm, mask_arm_atom, arms_p_index_in_ctx, arms_index_in_ctx = compose_context(
                        h_protein=h_arm_protein,
                        h_ligand=arm_v_feat,
                        pos_protein=arm_protein_pos,
                        pos_ligand=arm_pos,
                        batch_protein=arm_protein_batch,
                        batch_ligand=batch_arm,
                        ligand_atom_mask=None # debug
                    )
                    if arm_fc_bond_index is not None:
                        bond_index_in_all = arms_index_in_ctx[arm_fc_bond_index]
                    else:
                        bond_index_in_all = None
                    arm_bond_type = F.one_hot(arm_fc_bond_type, num_classes=self.num_bond_classes).float()
                    arm_h_bond = self.fusion_bond_emb(arm_bond_type)
                    arm_outputs = self.fusion_net(
                        h=h_all, x=pos_all, group_idx=None,
                        bond_index=bond_index_in_all, h_bond=arm_h_bond,
                        mask_ligand=mask_arm,
                        mask_ligand_atom=mask_arm_atom,
                        batch=batch_all,
                        return_all=return_all                    
                    )
                
                max_arm_id = ligand_arm_group_idx.max()
                unified_ligand_arm_scaffold_group_idx = torch.where(ligand_arm_atom_mask.bool(), 
                                                                    ligand_arm_group_idx,
                                                                    max_arm_id+1)
                # fuse arms atom feature
                arm_h = arm_outputs['h'][mask_arm_atom]
                aggregated_arm_h = scatter_sum(arm_h, batch_arm, dim=0)
                aggregated_arm_h = torch.cat([aggregated_arm_h, torch.zeros((1, self.hidden_dim)).to(aggregated_arm_h)], dim=0)
                atomwise_arm_h = torch.index_select(aggregated_arm_h, 0, unified_ligand_arm_scaffold_group_idx)

                init_ligand_v_feat = torch.cat([ligand_outputs['h'][mask_ligand_atom], atomwise_arm_h], dim=1)
                init_ligand_v = self.fusion_mlp(init_ligand_v_feat)

                # fuse pocket atom feature
                arms_protein_h = arm_outputs['h'][arms_p_index_in_ctx]
                ligand_protein_cumsum = torch.cumsum(torch.cat([torch.zeros(1).to(batch_protein), \
                    torch.unique(batch_protein, return_counts=True)[1]]), dim=0)
                cond_protein_idx = []
                for ligand_idx, arm_pocket_mask in zip(arm_to_ligand_index, arm_pocket_atom_masks):
                    cond_protein_idx.append(torch.where(arm_pocket_mask)[0] + ligand_protein_cumsum[ligand_idx])
                cond_protein_idx = torch.cat(cond_protein_idx)
                # pockets may exists overlap, aggregate overlapped atom feature
                aggregated_protein_h = scatter_mean(arms_protein_h, cond_protein_idx, dim=0)
                if len(p_index_in_ctx) > aggregated_protein_h.shape[0]:
                    aggregated_protein_h = torch.cat([aggregated_protein_h, torch.zeros(len(p_index_in_ctx) - aggregated_protein_h.shape[0], self.hidden_dim).to(aggregated_protein_h)])
                fused_protein_v = torch.cat([ligand_outputs['h'][p_index_in_ctx], aggregated_protein_h], dim=1)
                protein_v = self.fusion_protein_mlp(fused_protein_v)
            else:
                raise NotImplementedError
    ...
    def get_diffusion_loss(
            ...
            arm_pos, arm_v, arm_v_aux, batch_arm,
            arm_fc_bond_index, arm_fc_bond_type, batch_arm_bond,
            ligand_arm_group_idx, ligand_arm_atom_mask,
            arm_protein_pos, arm_protein_v, arm_protein_batch, arm_pocket_atom_masks, arm_to_ligand_index,

            prior_centers, prior_stds, prior_num_atoms, batch_prior,
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
        ...
        # >>> compute arm_outputs >>>
        arm_outputs, mask_arm_atom, arms_p_index_in_ctx = None, None, None
        if self.add_retrieved_feat:
            h_arm_protein = self.fusion_protein_atom_emb(arm_protein_v)
            arm_v = F.one_hot(arm_v, self.num_classes).float()
            arm_v_feat = self.fusion_atom_emb(arm_v)
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            preds = self(
                ...
                arm_pocket_atom_masks = arm_pocket_atom_masks, 
                arm_to_ligand_index = arm_to_ligand_index,
                mask_arm_atom = mask_arm_atom,
                arms_p_index_in_ctx = arms_p_index_in_ctx,
